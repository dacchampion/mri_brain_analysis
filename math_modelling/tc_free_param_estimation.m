...
T2_3_per_case = zeros(num_cases, 1);
...
for case_number=1:num_cases
    ...
    T2s = [20*ones(num_vox, 1), 80*ones(num_vox, 1), 2000*ones(num_vox, 1)];
    startx_mat = [sqrt(S0_vec), sqrt(T2s) ones(num_vox, 1)*0.13, ones(num_vox, 1)*0.42, ones(num_vox, 1)*0.35];
    ...
    for vox_ind=1:num_vox
        ...
        [parameter_hat, RESNORM] = fminunc('three_free_compartment', startx, h, true_S, TE_values);
        ...
        all_parameter_hat(vox_ind, 5:7) = parameter_hat(4:6).^2;
    end
    ...
    fprintf(strcat('Time elapsed: ', num2str(t_start), ', for case', num2str(case_number), ' with RESNORM=', num2str(RESNORMS(case_number)), '\n'));
end